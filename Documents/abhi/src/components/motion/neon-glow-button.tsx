...
const borderColorOf = (b: BorderConfig | undefined): string =>
...
const OUTSIDE_MASK: CSSProperties = {
...
export function NeonGlowButton({
  ...
  const fill = colors?.fill ?? fillProp ?? "#0b1f4d";
  const textColor = colors?.textColor ?? textColorProp ?? "#ffffff";
  const hoverFill = colors?.hoverFill ?? "#132e62";
  const hoverTextColor = colors?.hoverTextColor ?? "#93c5fd";

  const {
    color: glowColor = "#2563ff",
    size: glowSize = 6,
    blur: glowBlur = 5,
  } = glow;
  ...
  const fontStyles = (font ?? {}) as CSSProperties;

  const g1 = parseColor(glowColor, { r: 37, g: 99, b: 255, a: 1 });
  ...
  const rings: Ring[] = useMemo(() => {
    const g1Fade = cssRgba(g1, 0);
    const g2Fade = cssRgba(g2, 0);
    const g1Full = cssRgba(g1, 1);
    const g2Full = cssRgba(g2, 1);
    ...
    const mid: Ring = {
      ...
      hover: -98,
      focus: 442,
      gradient: `conic-gradient(${g1Fade}, ${cssRgba(g1, 0.75)}, ${g1Fade} 10%, ${g2Fade} 50%, ${cssRgba(g2, 0.75)}, ${g2Fade} 60%)`,
    };
    return [
      {
        inset: spread,
  ...
  const {
    ...
    hoverColor: iconHoverColor = "#93c5fd",
    side: iconSide = "right",
    size: iconSize = 14,
  ...
  return (
    <div
      ref={scope}
      className={cn("neon-glow-btn", className)}
      style={{
        minWidth: 80,
        minHeight: 40,
        position: "relative",
    ...
    >
      ...
      <Tag
        {...tagProps}
        ref={pillRef as React.Ref<HTMLButtonElement & HTMLAnchorElement>}
        onClick={onClick}
