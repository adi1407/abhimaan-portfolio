...
function buildItems(pool: DomeGalleryImage[], seg: number): DomeItem[] {
  ...
  const withSrc = pool.filter((img) => img.src);
  if (withSrc.length === 0) {
    return coords.map((c) => ({ ...c, src: "", alt: "" }));
  }
...
type Pose = { left: number; top: number; width: number; height: number };
...
export function DomeGallery({
  ...
  const originalTilePositionRef = useRef<Pose | null>(null);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        const overlay = enlargedOverlay as HTMLElement;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.style.cssText = `position:absolute;left:${overlayRelativeToRoot.left}px;top:${overlayRelativeToRoot.top}px;width:${overlayRelativeToRoot.width}px;height:${overlayRelativeToRoot.height}px;z-index:9999;border-radius: var(--enlarge-radius, 8px);overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.35);transition:all ${enlargeTransitionMs}ms ease-out;pointer-events:none;margin:0;transform:none;`;
